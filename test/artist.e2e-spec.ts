...
describe('ArtistController (e2e)', () => {
  ...
  it('POST /artists - should create an artist', async () => {
    const response = await request(app.getHttpServer()).post('/artists').send(dto).expect(201);

    expect(response.body).toMatchObject(dto);
    expect(response.body).toHaveProperty('id');
  });
  it('GET /artists/:id - should return 404 artist not found', async () => {
    await request(app.getHttpServer()).get('/artists/not-existing-id').expect(404);
  });
